...
namespace HeroSelection
{
    public class HeroSelectionScreenManager : MonoBehaviour
    {
        ...
        public void OpenScreen(HeroController heroController)
        {
            ...
            foreach (var hc in _heroControllers) // добавил
                hc.gameObject.SetActive(false);

            
            _currentHeroController = heroController;
            
            _currentHeroController.gameObject.SetActive(true); //добавил
        ...
        public void TryBuyHero()
        {
            ...
            if (_currencyManager.TryBuy(price))
            {
                ...
                _prefsManager.SavePurchasedHeroes(_currentHeroController.HeroSettings.Name);
                _heroSelectionScreenView.UpdateHeroSelectionView(_currentHeroController.HeroSettings);
            }
        }
        ...
        private void LoadPurchasedHeroesState()
        {
            var allHeroes = _prefsManager.LoadAllPurchasedHeroes();

            if (string.IsNullOrEmpty(allHeroes))
                return;

            var purchasedNames = allHeroes.Split(';');

            foreach (var hero in _heroControllers)
            {
                if (Array.Exists(purchasedNames, name => name == hero.HeroSettings.Name))
                {
                    hero.HeroSettings.WasBought = true;
                }
            }
        }
    }
